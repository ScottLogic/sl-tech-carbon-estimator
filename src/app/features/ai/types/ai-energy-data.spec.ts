import {
  AI_TASK_ENERGY_DATA,
  AI_PROVIDER_PUE_DATA,
  AI_ENERGY_DATA_CONFIG,
  allAiTaskArray,
  aiTaskArrayFromData,
} from './ai-energy-data';
...
describe('AI Energy Data Configuration', () => {
  describe('AI_TASK_ENERGY_DATA', () => {
    it('should contain energy data for all task types except mixed-usage', () => {
      ...
      expect(AI_TASK_ENERGY_DATA['image-generation'].stdev).toBe(3.31);
    });
  ...
  describe('AI_PROVIDER_PUE_DATA', () => {
    ...
    it('should contain exactly 8 providers', () => {
      const expectedProviders = ['openai', 'anthropic', 'google', 'microsoft', 'aws', 'meta', 'huggingface', 'other'];
  ...
  describe('Refactoring Benefits Demo', () => {
    ...
    it('should demonstrate the single source of truth principle', () => {
      ...
      Object.keys(dataSource).forEach(key => {
        expect(derivedArray).toContain(key as keyof typeof AI_TASK_ENERGY_DATA);
      });
